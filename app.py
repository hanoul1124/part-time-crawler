...
driver = webdriver.Chrome(executable_path='***/part-time-job/chromedriver', chrome_options=options)
...
try:
    for index, alba in enumerate(alba_list):
        if '분전' in alba.find_element_by_css_selector('td.recently > em').text:
    ...
    for index in new_alba_list:
        ...
        res2 = requests.post(send_url, headers=headers, data=json.dumps(body))
